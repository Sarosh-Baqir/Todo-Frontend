...
const Tasks = ({ searchQuery, isSidebarVisible, isMobile }) => {
  ...
  useEffect(() => {
    if (selectedDate) {
      const filtered = tasks.filter((task) => {
        ...
        return taskDate === new Date(selectedDate).toLocaleDateString();
      });
      setFilteredTasks(filtered);
    } else {
  ...
  return (
    <div>
      {isMobile && isSidebarVisible ? null : (
        <>
          <div className="flex justify-between items-center mb-4">
            {/* Calendar Input */}
            <input
              type="date"
              className="border-2 border-gray-400 rounded-lg px-3 py-2 w-40"
