...
import { UPDATE_TASK_ROUTE } from "../utils/constants";
import { toast } from "react-toastify";
import { AddTaskValidation } from "../validation/addTaskValidationSchema";

const EditTaskModal = ({ isOpen, onClose, taskToEdit, updateTaskList }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const hasChanges =
      ...
      task.dueDate !== taskToEdit.dueDate ||
      task.priority !== taskToEdit.priority ||
      task.status !== taskToEdit.status;

    if (!hasChanges) {
      toast.info("No changes detected. Task was not updated.");
      onClose();
      return;
    }
    ...
    try {
      const response = await axiosInstance.put(
        `${UPDATE_TASK_ROUTE}/${taskToEdit._id}`,
        task
      );
      toast.success(response?.data?.message || "Task updated successfully");
      updateTaskList(response?.data?.data);
      setTask({
        title: "",
        description: "",
        dueDate: "",
